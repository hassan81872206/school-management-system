        ...
        <form action="{{route('teachers.store')}}" method="POST">
            ...
            @error('classeID')
              ...
              <div id="class-container">
                <select class="form-select" name="classeID[]" aria-label="Default select example">
                    @foreach ($classes as $classe)
                        <option value="{{ $classe->classeID }}">{{ $classe->roomNbr }}</option>
                    @endforeach
                </select>
            </div>
            <button type="button" id="add-class" class="btn btn-primary" style="margin-top: 10px;">Add Another Classe</button>
    
            <br><br>
    
            <!-- Button and Select for Subjects -->
            ...
            <button type="button" id="add-subject" class="btn btn-primary" style="margin-top: 10px;">Add Another Subject</button>
    
            @if (session('error1'))
                <p style="color:red">{{session('error1')}}</p>
            @endif
